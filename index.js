...
async function evalPlaybillJobs() {
    return new Promise(async (funcResolve) => {
        ...
        const playbillJobs = playbillParse.querySelectorAll('.event-listing-table .bsp-component-content .pb-tile-wrapper a')
        const lastPlaybillJobSearched = localStorage.getItem('lastPlaybillJobSearched');

        let promise = new Promise(async (resolve) => {
            console.log(playbillJobs.length)
...
async function offstageJobDetails(url) {
    return new Promise(async (resolve) => {
        ...
        let website = "";
        try  { website = jobParse.querySelector('.itemContact h4 a').innerHTML } catch {}
        let details = {
